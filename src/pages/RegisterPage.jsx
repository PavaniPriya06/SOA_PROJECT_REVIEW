...
function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    let users = []
    try {
      users = JSON.parse(localStorage.getItem('bidvelocity_users') || '[]')
    } catch {
      users = []
    }

    if (users.some((user) => user.email === normalizedEmail)) {
      setSubmitMessage('An account with this email already exists.')
      return
    }
    ...
    localStorage.setItem('bidvelocity_users', JSON.stringify([...users, newUser]))
