...
function PaymentPage({ auctions, user, addNotification }) {
  ...
  useEffect(() => {
    ...
    paymentApi.getPaymentsByAuctionId(auction.id)
      .then((payments) => {
        const payment = payments.find((item) => String(item.bidderId) === String(user.id) && item.paymentStatus === 'SUCCESS')
        if (payment) {
  ...
  const handlePay = async () => {
    ...
    try {
      const result = await paymentApi.processPayment({ auctionId: auction.id, amount, bidderId: user.id })
      const payment = {
        paymentId: result.paymentId,
        auctionId: result.auctionId,
        amount: Number(result.amount),
        method,
        timestamp: result.paymentTime,
      }
      addNotification?.({
        type: 'PAYMENT',
